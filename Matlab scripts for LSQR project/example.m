...
I_originale = double(imread('hubble.tif')) / 255; %size(I)=(1518,1518)
...
% Recupero l'immagine con LSQR -- l'esempio qui usa 
% LSQR di MATLAB, che
% però non permette di scegliere il parametro lambda. 
I_recuperata = lsqr(@(v, flag) matvec(v, X, flag), I_sfocata(:));
